...
\begin{defn}
 Given some diagram $D: \mathcal{J} \to \mathcal{K}$ with type $\mathcal{J}$ in the category $\mathcal{K}$.
 A $\emph{cone}$ to the diagram $D$ is an object $N$ of the category $\mathcal{K}$ endowed with a family $\Psi_{X}: N \to D(X)$ of morphisms
...
TODO.. develope more with diagrams etc..
