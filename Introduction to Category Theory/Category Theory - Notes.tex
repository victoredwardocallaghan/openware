...
\begin{defn}[Natural Transformation]
 ...
 of $\eta$ at $X$, such that for every morphism $\varphi: X \to Y \in \mathcal{C}$ we have:
 \\
 $\eta_{Y} \circ F(\varphi) = G(g) \circ \eta_{X}$
\end{defn}
...
Recall that a natural transformation $\varphi: F \to G$ between two functors $F,G : \mathcal{C} \to \mathcal{K}$
is a total function $\varphi$ that maps $\text{Ob}(\mathcal{C}) \to \text{Hom}(\mathcal{K})$, with
$\varphi_{X}: F(X) \to G(X) \, \forall X \in \text{Ob}(\mathcal{C})$ such that the diagram commutes for each $\phi: X \to Y$.

\begin{tikzcd}
 F(X) \rar{\varphi_{X}} \dar[][color=red]{F(\phi)}
  & G(X) \dar[][color=red]{G(\phi)} \\
 F(Y) \rar{\varphi_{Y}}
  & G(Y)
\end{tikzcd}

\begin{exmp}
Common examples in programs include:
\begin{itemize}
 \item rev : List $\to$ List  =  reversing a list
 \item fla : Tree $\to$ List  =  flattening a tree
 \item len : List $\to$ Int   =  length of a list
\end{itemize}
\end{exmp}
