...
$\alpha: C \to A$ and $\beta: C \to B$, such that the following prperty holds:
...
We denote the product of $A$ and $B$ as: $A \prod B$ or sometimes $A \bigotimes B$.
...
$\alpha: A \to C$ and $\beta: B \to C$, such that the following prperty holds:
...
We denote the coproduct, or `sum', of $A$ and $B$ as: $A \coprod B$ or sometimes $A \bigoplus B$.
...
\begin{exmp}[Initial algebra]
 Consider a functor $F: \textbf{Set} \to \textbf{Set}$, in particular, $F: X \to \{ \; \} \bigoplus X$.
 Then the set $\N$ endowered with the function $[zero,succ]: \{ \; \} \bigoplus \N \to N$, is a $F$-algebra.
 This initial algebra is denoted by the pair $(\N, [zero,succ])$.
\end{exmp}

Here the singleton set $\{ \; \}$ denotes the $\emph{terminal object}$ and $\bigoplus$ as the $\emph{coproduct}$.
